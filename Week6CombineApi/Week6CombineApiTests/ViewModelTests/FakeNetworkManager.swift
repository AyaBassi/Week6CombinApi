...
class FakeNetworkManager : CombineNetworkableProtocol {
    ...
    func getListFromAPI<T>(url: URL) -> AnyPublisher<T, Error> where T : Decodable {
        ...
        do {
            ...
            switch error {
            case is URLError :      return Fail(error:NetworkErrorEnum.invalidUrlError).eraseToAnyPublisher()
            case is DecodingError:
                //NetworkErrorEnum = NetworkErrorEnu
                return Fail(error: NetworkErrorEnum.parsingError).eraseToAnyPublisher()
            default:
